...
const GitSearch = () => {
	...
	let fetchUser = () => {
		var nameValue = (document.getElementById("site-search") as HTMLInputElement)
			.value;
				...
				{profileHeader && repositories && (
					<>
						...
						{profileHeader !== "No user found!" && (
							...
							>
								{!showRepos ? "Show Repositories" : "Hide Repositories"}
							</Button>
						)}
					</>
				)}
							...
							repoArr.map((repoName, id) => {
								return (
									<span key={id} className="repos">
										<Typography align="center">{repoName}</Typography>
									</span>
								);
							})}
					</aside>
